...
const SESSION_TEMPLATES = [
  { id: "template-shot", name: "Shot-Stopping Day", notes: "Built from the drill library template.", drillIds: ["ss5", "fw10", "ss7", "rx2"] },
  { id: "template-crosses", name: "Crosses & Box Command", notes: "Built from the drill library template.", drillIds: ["fw8", "cr1", "cr2", "cr3"] },
  { id: "template-distribution", name: "Distribution Focus", notes: "Built from the drill library template.", drillIds: ["ds2", "ds3", "ds4", "ds6"] },
  { id: "template-match", name: "Pre-Match Routine", notes: "Built from the drill library template.", drillIds: ["rc5", "sp5", "mn1"] },
];
...
function buildTemplateSession(template) {
  return {
    id: uid(),
    name: template.name,
    ...
    drills: template.drillIds.map(id => {
      ...
      return drill ? { ...drill, _uid: uid() } : null;
    }).filter(Boolean),
  };
}

function getSessionWarnings(session) {
  const totalTime = session.drills.reduce((sum, drill) => sum + (drill.dur || 0), 0);
  const highCount = session.drills.filter(drill => drill.int === "High" || drill.int === "Max").length;
  const warnings = [];
  if (totalTime < 30) warnings.push("Very short session");
  if (totalTime > 90) warnings.push("Long session");
  if (highCount >= 3) warnings.push("High intensity load");
  if (!session.drills.length) warnings.push("No drills added");
  return warnings;
}
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .drill { border: 1px solid #ddd; border-radius: 8px; padding: 12px; margin-bottom: 12px; }
          .tag { display: inline-block; font-size: 12px; padding: 2px 8px; background: #eef2ff; border-radius: 999px; margin-right: 6px; }
          .section { margin-top: 8px; }
        </style>
      </head>
      <body>
        <h1>${session.name}</h1>
        <div class="meta">Duration: ${totalTime} min | Drills: ${session.drills.length}</div>
        ${session.notes ? `<p><strong>Session Notes:</strong> ${session.notes}</p>` : ""}
        ${session.drills.map((drill, index) => `
          <div class="drill">
            <h3>${index + 1}. ${drill.name}</h3>
            <div class="tag">${drill.int}</div>
            <div class="tag">${drill.dur} min</div>
            <div class="section">${drill.desc}</div>
            ${drill.reps ? `<div class="section"><strong>Reps:</strong> ${drill.reps}</div>` : ""}
            ${drill.cp ? `<div class="section"><strong>Coaching:</strong> ${drill.cp}</div>` : ""}
            ${drill.eq?.length ? `<div class="section"><strong>Equipment:</strong> ${drill.eq.join(", ")}</div>` : ""}
          </div>
        `).join("")}
      </body>
    </html>
...
function DrillCard({ drill, onAdd, expanded, onToggle, isFavorite, onToggleFavorite, recentRank }) {
  ...
  return (
    <div style={{ border: "1px solid " + colors.border, borderRadius: 8, background: colors.card, overflow: "hidden", borderLeft: "4px solid " + (cat?.color || colors.accent) }}>
      <div style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={onToggle}>
        ...
        {recentRank ? <Badge color={colors.warmLight} textColor="#92400e">Recent #{recentRank}</Badge> : null}
        ...
        <button type="button" onClick={e => { e.stopPropagation(); onToggleFavorite(drill.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: isFavorite ? "#dc2626" : colors.textMuted, padding: 2 }}>
          <Heart size={14} fill={isFavorite ? "#dc2626" : "none"} />
        </button>
        {onAdd && <button type="button" onClick={e => { e.stopPropagation(); onAdd(drill); }} style={{ background: colors.accent, color: "#fff", border: "none", borderRadius: 4, padding: "4px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>+ Add</button>}
      </div>
      {expanded && (
...
function SessionBuilder({ session, onUpdate, onBack, onAddDrill, onShare }) {
  ...
  const onDragStart = (event, index) => event.dataTransfer.setData("text/plain", String(index));
  ...
  const addLog = () => {
    const nextLogs = [...(session.logs || []), { id: uid(), date: new Date().toISOString().slice(0, 10), note: "" }];
    onUpdate({ ...session, logs: nextLogs });
  };
  const updateLog = (id, field, value) => onUpdate({
    ...session,
    logs: (session.logs || []).map(log => log.id === id ? { ...log, [field]: value } : log),
  });
  const removeLog = (id) => onUpdate({
    ...session,
    logs: (session.logs || []).filter(log => log.id !== id),
  });

  return (
    <div>
      <button type="button" onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: colors.textMuted, marginBottom: 14, fontSize: 13, fontWeight: 600 }}><ArrowLeft size={16} /> Back to Sessions</button>
      ...
      {warnings.length > 0 && (
        <div style={{ marginBottom: 14, padding: 12, borderRadius: 8, background: colors.warmLight, color: "#92400e", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <AlertCircle size={16} />
          {warnings.map(warning => <Badge key={warning} color="rgba(255,255,255,0.65)" textColor="#92400e">{warning}</Badge>)}
        </div>
      )}
      ...
      {session.drills.map((d, i) => {
        ...
        return (
          <div key={d._uid || i} draggable onDragStart={event => onDragStart(event, i)} onDragOver={event => event.preventDefault()} onDrop={event => onDrop(event, i)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", border: "1px solid " + colors.border, borderRadius: 8, marginBottom: 6, background: colors.card, borderLeft: "4px solid " + (cat?.color || "#ccc") }}>
            <GripVertical size={14} color={colors.textMuted} />
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <button type="button" onClick={() => moveDrill(i, -1)} disabled={i === 0} style={{ background: "none", border: "none", cursor: i === 0 ? "default" : "pointer", opacity: i === 0 ? 0.2 : 0.6, padding: 0, fontSize: 10 }}>{"\u25B2"}</button>
              <button type="button" onClick={() => moveDrill(i, 1)} disabled={i === session.drills.length - 1} style={{ background: "none", border: "none", cursor: i === session.drills.length - 1 ? "default" : "pointer", opacity: i === session.drills.length - 1 ? 0.2 : 0.6, padding: 0, fontSize: 10 }}>{"\u25BC"}</button>
            </div>
            ...
            <button type="button" onClick={() => removeDrill(i)} style={{ background: "none", border: "none", cursor: "pointer", color: colors.danger, padding: 4 }}><Trash2 size={14} /></button>
          </div>
        );
      })}
      <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid " + colors.border }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <h3 style={{ margin: 0, fontSize: 15 }}>Session Log</h3>
          <Button onClick={addLog} variant="secondary" size="sm"><Plus size={14} /> Add Log Note</Button>
        </div>
        {(session.logs || []).length === 0 && <div style={{ fontSize: 13, color: colors.textMuted }}>No dated notes yet. Add one after a training day to remember how it actually went.</div>}
        {(session.logs || []).map(log => (
          <div key={log.id} style={{ display: "grid", gridTemplateColumns: "160px 1fr auto", gap: 10, alignItems: "start", marginBottom: 10 }}>
            <Input label="Date" type="date" value={log.date} onChange={value => updateLog(log.id, "date", value)} />
            <Input label="What Happened" multiline rows={2} value={log.note} onChange={value => updateLog(log.id, "note", value)} placeholder="What worked, what to repeat, what to change..." />
            <button type="button" onClick={() => removeLog(log.id)} style={{ marginTop: 24, background: "none", border: "none", cursor: "pointer", color: colors.danger }}><Trash2 size={14} /></button>
          </div>
        ))}
      </div>
    </div>
...
function SessionCard({ session, onOpen, onDuplicate, onDelete, onPrint, onShare }) {
  ...
  return (
    <div style={{ border: "1px solid " + colors.border, borderRadius: 8, padding: 14, background: colors.card, cursor: "pointer", transition: "box-shadow 0.15s" }} onClick={onOpen}
      ...
      {warnings.length > 0 && <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>{warnings.map(warning => <Badge key={warning} color={colors.warmLight} textColor="#92400e">{warning}</Badge>)}</div>}
    </div>
  );
}
...
export default function GKCoachPlanner() {
  ...
  const filteredDrills = useMemo(() => {
    return DRILL_LIBRARY.filter(d => {
      ...
      const me = libEquipment === "All" || d.eq?.includes(libEquipment);
      const md = libDuration === "All" || (libDuration === "<=10" ? d.dur <= 10 : libDuration === "11-15" ? d.dur >= 11 && d.dur <= 15 : d.dur >= 16);
      const mf = !showFavoritesOnly || favoriteIds.includes(d.id);
      const mr = !showRecentOnly || recentIds.includes(d.id);
      return ms && mc && mi && me && md && mf && mr;
    });
  }, [libSearch, libCat, libInt, libEquipment, libDuration, showFavoritesOnly, showRecentOnly, favoriteIds, recentIds]);
  ...
  const createSession = () => {
    const s = { id: uid(), name: "New Session", drills: [], notes: "", logs: [] };
  ...
  const createFromTemplate = (template) => {
    const session = buildTemplateSession(template);
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", justifyContent: "center", alignItems: "center" }}>
      <div style={{ background: "#fff", borderRadius: 12, width: "90%", maxWidth: 800, maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid " + colors.border, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button type="button" onClick={() => setAddingDrills(false)} style={{ background: "none", border: "none", cursor: "pointer" }}><X size={20} /></button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "10px 20px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filteredDrills.map(d => <DrillCard key={d.id} drill={d} onAdd={addDrillToSession} expanded={expandedDrill === d.id} onToggle={() => setExpandedDrill(expandedDrill === d.id ? null : d.id)} isFavorite={favoriteIds.includes(d.id)} onToggleFavorite={toggleFavorite} recentRank={recentIds.indexOf(d.id) >= 0 ? recentIds.indexOf(d.id) + 1 : null} />)}
          </div>
